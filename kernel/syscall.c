...
extern uint64 sys_stat_syscall(void);
...
static uint64 (*syscalls[])(void) = {
  ...
  [SYS_stat_syscall]    sys_stat_syscall,
  [SYS_times] sys_times,
  [SYS_uname] sys_uname,
};
...
syscall(void)
{
  ...
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    syscall_counts[num]++; // count the syscall
...
uint64 sys_uname(void)
{
  ...
  struct utsname ans = {
    ...
    "pku_os_lab",            // nodename
    "mark1",                 // release
    "xv6-k210-qemu",        // version
    "riscv64",               // machine
    "arsenal.com"          // domainname
};
